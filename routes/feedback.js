...
router.post('/', requireAuth, async (req, res) => {
  const { message, title, rating, category, page_url } = req.body ?? {};
  ...
  try {
    await db.prepare(
      'INSERT INTO feedback (user_id, message, rating, page_url, category) VALUES ($1, $2, $3, $4, $5)'
    ).run(userId, message.trim(), ratingVal, pageUrl, categoryVal);
  } catch (err) {
