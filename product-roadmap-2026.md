...
*Strategic review and roadmap based on codebase audit and competitive analysis. Updated 2026-05-26 (Phases 1–7 quality initiative shipped — vault onboarding, voice signal, smarter input, two-stage generation, feedback loop, substance check, hook strategy).*
...
- **Vault-First Onboarding (Phase 1)** — New Step 3 between Q&A and first post: "Upload your best work." Accepts PDF/DOCX/TXT/URL; skippable. If uploaded, first generated post uses the vault path. Empty-state quality-lift banner on generate page for users with zero vault docs *(shipped 2026-05-26)*
- **Voice Signal from Real Writing (Phase 2)** — Website extraction now crawls up to 3 blog/article pages and stores full text (`website_articles_text`); fed into `extractVoiceDNAFromQA()` for richer voice extraction. Writing sample paste added as an optional step in onboarding between website and Q&A. Voice extraction upgraded to Sonnet (from Haiku) and now extracts sentence rhythm, vocabulary tier, opening move, argument structure, and what the user *never* says. Voice profile summary (extracted traits) shown in settings *(shipped 2026-05-26)*
- **Smarter Generate Page Input (Phase 3)** — Niche-specific textarea placeholders pulled from `input_examples` (JSON array generated per user at onboarding via Haiku; stored in `user_profiles`). "Help me find my story →" collapsible micro-interview panel: 3 questions assemble a rich idea input and fill the textarea. Specificity nudge extended to be niche-aware — warns with field-specific language instead of generic number nudge *(shipped 2026-05-26)*
- **Two-Stage Generation Architecture (Phase 4)** — `ideaToPost()` now runs Haiku first (Stage 1: finds single strongest tension, determines narrative arc, selects hook archetype → returns `{ tension, arc, archetype, hook_draft }`), then Sonnet second (Stage 2: writes the post using the blueprint as fixed structure; system prompt contains only voice DNA + formatting rules + blueprint). Processing screen shows "Found the tension: [arc]" from Stage 1 output before full post arrives *(shipped 2026-05-26)*
- **Feedback Loop (Phase 5)** — Auto-save drafts: debounced PATCH `/api/posts/:id` fires 2s after user stops typing in the editor. Multi-dimensional change detection replaces single Levenshtein threshold: hook change (opening sentence differs), vocabulary change (3+ word substitutions), length change (≥20% shorter/longer), general (>30% Levenshtein). Multiple voice rules extracted per save. "Voice is learning →" indicator in editor sidebar shows 3 most recent refinements. `ai_content TEXT` added to `generated_posts` to preserve original AI output before edits *(shipped 2026-05-26)*
- **Multi-Dimensional Substance Check (Phase 6)** — `assessInputQuality()` now checks four dimensions: `hasSpecific` (number/result/timeframe), `hasTension` (surprising outcome/failure/unpopular view), `hasRelevance` (niche + audience match), `hasNovelty` (not a cliché angle). Tiered response: 4/4 = generate; 3/4 = generate with quiet hint; 2/4 = specific coaching note; 1/4 = full warning with niche example. "Generate anyway" replaced with "Improve my input →" which opens the Phase 3 micro-interview. Vault-sourced posts always skip the check *(shipped 2026-05-26)*
- **Personalized Hook Strategy (Phase 7)** — Content pillars: 2-3 topic focus areas derived at onboarding from niche + Q1/Q2 via Haiku; stored as `content_pillars` on `user_profiles`; editable in settings with "✦ Suggest pillars" button. Archetype preference tracking: every published post (immediate + scheduled) increments a per-archetype count in `user_archetype_preference` (JSON map). Hook selection bias: `selectHook()` injects a USER SIGNATURE STYLE block into the Haiku classification prompt when the user has publish history, nudging toward their established archetypes on ambiguous inputs. Archetype coaching panel in settings shows top-3 signature hooks with publish counts *(shipped 2026-05-26)*
