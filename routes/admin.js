...
router.get('/idea-engine-report', requireAdminPassword, (req, res) => {
  (async () => {
    ...
    const events = await db.prepare(`
      ...
      WHERE  (event_type LIKE 'idea_%' OR event_type = 'streak_incremented')
        AND  created_at > NOW() - INTERVAL '30 days'
      GROUP  BY event_type
      ORDER  BY event_type
    `).all();

    // Phase 2 — queue funnel: saved → sitting in queue → written from queue.
    // "queue → write" = cards that passed through 'saved' (idea_card_saved
    // event) and ended up 'generated'.
    const queueFunnel = await db.prepare(`
      SELECT COUNT(*) FILTER (WHERE ic.status = 'saved')    AS in_queue_now,
             COUNT(*) FILTER (WHERE ic.status = 'archived') AS archived,
             COUNT(*) FILTER (WHERE ic.status = 'generated'
               AND EXISTS (SELECT 1 FROM platform_events pe
                           WHERE pe.event_type = 'idea_card_saved'
                             AND (pe.metadata->>'idea_card_id') = ic.id::text))
                                                            AS queue_to_write
      FROM   idea_cards ic
      WHERE  ic.served_on > CURRENT_DATE - INTERVAL '30 days'
    `).get();

    // Phase 2 — daily-ideas email sends (email_log is the source of truth;
    // idea_email_sent events carry the per-card lineage in the breakdown above)
    const emailStats = await db.prepare(`
      SELECT COUNT(*)                AS sent,
             COUNT(DISTINCT user_id) AS recipients
      FROM   email_log
      WHERE  template = 'daily-ideas'
        AND  sent_at > NOW() - INTERVAL '30 days'
    `).get();

    // Phase 2 — consistency streaks (lazy reset: alive = acted since yesterday)
    const streaks = await db.prepare(`
      SELECT COUNT(*) FILTER (WHERE streak_last_date >= CURRENT_DATE - INTERVAL '1 day') AS active_streaks,
             MAX(streak_count) FILTER (WHERE streak_last_date >= CURRENT_DATE - INTERVAL '1 day') AS longest_active,
             ROUND(AVG(streak_count) FILTER (WHERE streak_last_date >= CURRENT_DATE - INTERVAL '1 day'), 1) AS avg_active
      FROM   user_profiles
    `).get();

    // Phase 2 — cohort view by join month: is the idea engine hooking newer
    // cohorts harder? Last-30d posts + idea share per signup month (6 months).
    const cohorts = await db.prepare(`
      SELECT to_char(date_trunc('month', up.created_at), 'YYYY-MM') AS cohort,
             COUNT(DISTINCT up.user_id)                              AS users,
             COUNT(gp.id)                                            AS posts,
             COUNT(gp.id) FILTER (WHERE gp.idea_card_id IS NOT NULL) AS posts_from_idea
      FROM   user_profiles up
      LEFT   JOIN generated_posts gp
             ON gp.user_id = up.user_id
            AND gp.created_at > NOW() - INTERVAL '30 days'
